...
public class ExcelExporter extends Exporter {
	...
	protected void addColumnFacets(DataTable table, Sheet sheet,
		...
		int sheetRowIndex = columnType.equals(Exporter.ColumnType.HEADER) ? 0
	...
	protected void addColumnValue(Row row, UIComponent component) {
		String value = component == null ? "" : exportValue(
		...
		for (UIComponent component : components) {
			if (component.isRendered()) {
				...
				if (component instanceof UIRepeat) {
					UIRepeat hallo = (UIRepeat) component;
					value = "";
					Object object = hallo.getValue();
					try {
						List<Task> value3 = (List<Task>) object;
						for (Task task : value3) {
							value = value + " " + task.getName();
						}
					} catch (Exception e) {

					}
					try {
						List<Preventive> value3 = (List<Preventive>) object;
						for (Preventive prevetnive : value3) {
							value = value + " " + prevetnive.getPreventiveId();
						}
					} catch (Exception e2) {
					...
					try {
						List<Artikull> value3 = (List<Artikull>) object;
						for (Artikull artikull : value3) {
							value = value + " " + artikull.getArtikullId();
						}
					} catch (Exception e3) {
